...
- `npm run metricas` imprime las visitas (Vercel Web Analytics: total, por día, país, dispositivo, navegador, procedencia) y el resumen de las preguntas registradas (total, distintas, cuántas fueron al modelo, costo, por día, artículos más citados). `-- --dias 7`, `-- --desde AAAA-MM-DD --hasta AAAA-MM-DD`, `-- --json`.
...
- **Métricas de visitas.** Vercel Web Analytics, activado el 7 de septiembre de 2026, sin cookies ni datos personales: visitantes, vistas de página, país y dispositivo. `npm run metricas` las imprime en la terminal junto con el resumen de la tabla `preguntas` (opciones: `-- --dias 7`, `-- --desde 2026-09-01 --hasta 2026-09-30`, `-- --json`); necesita la sesión de `vercel login` (o `VERCEL_TOKEN`) y `DATABASE_URL`. También están en vercel.com → proyecto `pavaresponde` → Analytics. `src/components/Analytics.tsx` manda las vistas y descarta las de `/admin`, que son del personal. La herramienta MCP de Vercel `get_web_analytics` responde 404 para este proyecto aunque Analytics está activo: usa el comando.
